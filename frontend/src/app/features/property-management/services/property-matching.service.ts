...
export class PropertyMatchingService {
  ...
  getMatchScoreColorClass(score: number): string {
    if (score >= 80) return 'text-[var(--color-success)] bg-[var(--color-success-soft)]';
    if (score >= 60) return 'text-[var(--primary)] bg-[var(--accent-soft)]';
    if (score >= 40) return 'text-[var(--color-warning)] bg-[var(--color-warning-soft)]';
